                ...
                return `
                    <div class="seed-row">
                        ...
                        <div class="seed-fields">
                            ...
                            <div class="seed-field">
                                <label>Modo</label>
                                <select data-focus-key="insumo-${insumo.id}-product-${index}-mode" oninput="updateProductField(${insumo.id}, ${index}, 'mode', this.value)">
                            ...
                            <div class="seed-field">
                                <label>Emb.</label>
                                <select data-focus-key="insumo-${insumo.id}-product-${index}-bagSize" oninput="updateProductField(${insumo.id}, ${index}, 'bagSize', this.value)">
                                    ${isSoy ? `
                ...
                return `
                    <div class="product-row">
                        <div class="product-fields">
                            ...
                            <div class="field-group">
                                <label>Unid.</label>
                                <select data-focus-key="insumo-${insumo.id}-product-${index}-unit" oninput="updateProductField(${insumo.id}, ${index}, 'unit', this.value)" style="width: 55px;">
...
function getConsolidationStoredQuantity(key, fallbackQuantity) {
    const savedQuantity = state.consolidation.quantities[key];
    if (savedQuantity === undefined || savedQuantity === '') {
        return fallbackQuantity;
    }

    const parsed = parseFloat(savedQuantity);
    return Number.isNaN(parsed) ? fallbackQuantity : parsed;
}
...
function renderConsolidation() {
    ...
    let grandTotal = 0;
    products.forEach(p => {
        const currentQuantity = getConsolidationStoredQuantity(p.key, p.quantity);
                ...
                <td style="text-align: right;">
                    <div class="quantity-cell">
                        <input type="number" 
                               ...
                               value="${product.currentQuantity.toFixed(3)}" 
